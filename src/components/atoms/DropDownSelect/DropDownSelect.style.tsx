...
export const SelectField = styled.select<ISelectField & { theme: TTheme }>`
  ...
  ${({ theme }) => css`
    background-image: ${`url("data:image/svg+xml;utf8,<svg viewBox='0 0 18 18' xmlns='http://www.w3.org/2000/svg' width='14' height='14' fill='${theme.color.secondary[4].replace(
      '#',
      '%23'
  ...
  color: ${({ theme, value }) => (value ? theme.color.secondary[0] : theme.color.secondary[4])};
  border: none;
  width: 100%;
  font-weight: 500;
  user-select: none;
  ...
  option {
    ...
    &:disabled {
      background: ${({ theme }) => theme.color.secondary[9]};
      color: ${({ theme }) => theme.color.secondary[4]};
    }
  }
`;
