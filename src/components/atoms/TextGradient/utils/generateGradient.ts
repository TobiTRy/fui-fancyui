import { TGradientColor, TTextGradientProps } from '@/components/atoms/TextGradient/TextGradient.model';
...
export const generateGradient = (props: Omit<TTextGradientProps, 'children'>) => {
  ...
  switch (orientation) {
    ...
    case 'radial':
      gradient = `radial-gradient(circle at ${position || 'center'}, ${colorString})`;
      break;
    case 'elliptical':
      gradient = `radial-gradient(ellipse at ${position || 'center'}, ${colorString})`;
      break;
    case 'repeatLinear': {
      const linearPosition = position ? getLinearGradientPosition(position) : `${degree || 90}deg`;
    ...
    case 'repeatRadial':
      gradient = `repeating-radial-gradient(circle at ${position || 'center'}, ${colorString})`;
      break;
    case 'repeatElliptical':
      gradient = `repeating-radial-gradient(ellipse at ${position || 'center'}, ${colorString})`;
      break;
    default:
