...
To use the `FancyPill` in your project, follow these steps:

1. Import the `FancyPill` component where you intend to use it:

```jsx
import { FancyPill } from 'fui-fancyui';
```

2. If using TypeScript, you can import the type definitions for prop validation:

```jsx
import { TFancyPill, TFancyPillWithHTMLAttrs } from 'fui-fancyui';
```
