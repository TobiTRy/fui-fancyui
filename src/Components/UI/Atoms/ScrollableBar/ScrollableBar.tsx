...
export default function ScrollableBar(props: IScrollableBar) {
  const { childrenArray, secondBar } = { ...defaultProps, ...props };

  const scrollAbleBar = useRef<HTMLDivElement>(null);
  const activateScrollbar = childrenArray?.length! > 4 ? true : false;
...
const defaultProps: IScrollableBar = {
  childrenArray: [
    <FancyBottomBarIcon label="hi"  key={1} icon={svg} />,
    <FancyBottomBarIcon label="hi1" key={2} icon={svg} />,
    <FancyBottomBarIcon label="hi2" key={3} icon={svg} />,
    <FancyBottomBarIcon label="hi3" key={4} icon={svg} />,
    <FancyBottomBarIcon label="hi4" key={5} icon={svg} />,
  ],
};
